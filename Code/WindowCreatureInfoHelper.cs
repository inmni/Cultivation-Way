...
namespace Cultivation_Way
{
    class WindowCreatureInfoHelper
    {
		...
		{//加载特质
			...
			num++;
			loadRealmButton(data.actorID, num, count,data.level);
			num++;
			...
			Localization.setLocalization("trait_element", Main.instance.actorToMoreStats[data.actorID].element.element.name+"灵根");
			Localization.setLocalization("trait_cultivationBook", Main.instance.actorToMoreStats[data.actorID].family.cultivationBook.bookName);
			if (data.traits != null)
			{
		...
		internal void loadRealmButton(string actorID, int pIndex,int pTotal,int level)
        {
			//境界贴图名由路线+等级组成
			//功法贴图固定
			//灵根贴图固定
			...
			if (Main.instance.actorToMoreStats[actorID].spells.Count != 0)
			{
			...
			Sprite sprite = Sprites.LoadSprite(Main.mainPath+"/EmbededResources/icons/traits/"+Main.instance.actorToMoreStats[actorID].cultisystem+"_"+1+".png");
			traitButton.GetComponent<Image>().sprite = sprite;
			Localization.setLocalization("trait_realm", ((CultisystemLibrary)AssetManager.instance.dict["cultisystem"]).get(Main.instance.actorToMoreStats[actorID].cultisystem).realms[level-1]);
		...
		internal void loadCultivationBookButton(string actorID, int pIndex, int pTotal, int level)
		{
			...
			TraitButton traitButton = UnityEngine.Object.Instantiate<TraitButton>(instance.prefabTrait, instance.traitsParent);
			StringBuilder description = new StringBuilder();
			if (Main.instance.actorToMoreStats[actorID].family.cultivationBook.stats[19].spells.Count != 0)
			{
				description.Append("法术\n");

				foreach (ExtensionSpell spell in Main.instance.actorToMoreStats[actorID].family.cultivationBook.stats[19].spells)
				{
					ExtensionSpellAsset spellAsset = AddAssetManager.extensionSpellLibrary.get(spell.spellAssetID);
					description.Append("\n" + spellAsset.name);
				}
